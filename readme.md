...
Script for generating a gadget with a fixed completeness using an LP solver. Pick between Gurobi (fast but uses floating point numbers) or qsopt_ex (slower but computes optimum exactly).
Running this script requires installing;
* Gurobipy
* (Optionally) qsopt_ex (https://www.math.uwaterloo.ca/~bico/qsopt/ex/)
* (Optionally) License for Gurobi (https://www.gurobi.com/)
